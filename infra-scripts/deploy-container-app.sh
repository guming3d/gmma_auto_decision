...
if [[ ! -f "${REPO_ROOT}/Dockerfile" ]]; then
  echo "Dockerfile not found in ${REPO_ROOT}. Please generate it before deploying." >&2
  exit 1
fi

if [[ ! -f "${REPO_ROOT}/.streamlit/secrets.toml" ]]; then
  echo ".streamlit/secrets.toml is missing; the container needs it for Streamlit auth." >&2
  exit 1
fi
...
usage() {
  cat <<'EOF'
Bootstrap an Azure Container Apps deployment for the Streamlit dashboard.
Configuration defaults are loaded from .env at the repository root.

Required environment variables:
  AZ_SUBSCRIPTION_ID             Azure subscription ID
  AZ_RESOURCE_GROUP              Resource group that will contain all assets
  AZ_LOCATION                    Azure region (e.g. eastus)
  AZ_CONTAINERAPPS_ENVIRONMENT   Container Apps environment name
  AZ_CONTAINER_APP_NAME          Name for the Container App instance
  AZ_ACR_NAME                    Azure Container Registry (will be created if missing)
...
Optional environment variables:
  ...
  AZ_MAX_REPLICAS                Container Apps max replicas (default: 1)
...
Example:
  ...
  AZ_LOCATION=eastus \
  AZ_CONTAINERAPPS_ENVIRONMENT=gmma-env \
...
export AZ_MAX_REPLICAS="${AZ_MAX_REPLICAS:-1}"
...
echo "Ensuring container registry ${AZ_ACR_NAME} exists..."
...
echo "Ensuring Container Apps environment ${AZ_CONTAINERAPPS_ENVIRONMENT} exists..."
if ! az containerapp env show --name "${AZ_CONTAINERAPPS_ENVIRONMENT}" --resource-group "${AZ_RESOURCE_GROUP}" >/dev/null 2>&1; then
  az containerapp env create \
    --name "${AZ_CONTAINERAPPS_ENVIRONMENT}" \
    --resource-group "${AZ_RESOURCE_GROUP}" \
    --location "${AZ_LOCATION}" \
...
if az containerapp show --name "${AZ_CONTAINER_APP_NAME}" --resource-group "${AZ_RESOURCE_GROUP}" >/dev/null 2>&1; then
  ...
  az containerapp create \
    --name "${AZ_CONTAINER_APP_NAME}" \
    --resource-group "${AZ_RESOURCE_GROUP}" \
    --environment "${AZ_CONTAINERAPPS_ENVIRONMENT}" \
    --image "${IMAGE_URI}" \
    --registry-server "${ACR_LOGIN_SERVER}" \
    --registry-username "${ACR_USERNAME}" \
    --registry-password "${ACR_PASSWORD}" \
